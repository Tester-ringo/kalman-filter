...
def update_multiple(x: ARRAY_N_1, 
                    p: ARRAY_N_N, 
                    f: FUNC_VECTOR_N_TO_N, 
    ...
    weight                  = np.concatenate([[k/(n+k)], np.full(2*n, 0.5/(n+k))]).reshape(1, 2*n+1) #それぞれのシグマポイントの重みづけ size:(1, 2n+1)
    sigma_point_transition  = np.asarray(list(map(f, sigma_point.T))).T #推移関数で推移させる (もう少し良い実装は無いのだろうか) size:(n, 2n+1)
    x_                      = np.add.reduce(sigma_point_transition*weight, axis=1).reshape(n, 1) #推移後の状態変数の予測値 size:(n, 1)
    ...
    y_                      = np.add.reduce(sigma_point_observation*weight, axis=1).reshape(n, 1) #推定観測値を計算 size:(m, 1)
